...
from mcp_electrico import motor_starting_profiles
...
def _profile_package() -> dict:
    return json.loads(
        (ROOT / "examples" / "p13_motor_starting_profile_stage2.json").read_text(encoding="utf-8")
    )
...
def test_p13c_first_point_must_match_p13a_initial_starting_data():
    ...
    assert any(issue["code"] == "P13C021" for issue in result["issues"])
...
def test_p13c_times_must_be_strictly_increasing():
    ...
    assert result["validation_status"] == "BLOCKED_STARTING_PROFILE_INPUTS"
    assert any(issue["code"] == "P13C017" for issue in result["issues"])


def test_p13c_executes_declared_points_in_isolated_context():
...
def test_p13c_profile_execution_is_repeatable():
    ...
    first = motor_starting_profiles.ejecutar_perfiles(manifest, package)
    ...
    assert first_profile["status"] == second_profile["status"]
