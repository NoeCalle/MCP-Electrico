...
def test_p13b_readiness_materializes_without_solving_and_without_defaults():
...
def test_p13b_blocks_if_base_network_retains_engine_default():
    manifest = _manifest()
...
def test_p13b_executes_static_start_in_isolated_context():
    ...
    assert model["semantics"] == "CONSTANT_IMPEDANCE_EQUIVALENT_FROM_EXPLICIT_SUPPLY_LINE_RMS_CURRENT_AND_FUNDAMENTAL_DISPLACEMENT_PF"
...
def test_p13b_starting_method_label_does_not_change_network_result():
    dol = _manifest()
    alt = deepcopy(dol)
