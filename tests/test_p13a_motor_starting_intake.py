...
def test_p13a_generic_motor_fixture_is_ready_without_calculation():
    result = motor_starting_intake.evaluar_admision_motor(_manifest())

    assert result["schema"] == "MCP_ELECTRICO_P13A_MOTOR_STARTING_INTAKE_V1"
...
def test_p13a_never_derives_starting_current_from_starting_method():
    manifest = _manifest()
...
def test_p13a_requires_explicit_running_load_replacement_semantics():
    manifest = _manifest()
    ...
    assert result["intake_status"] == "BLOCKED_MOTOR_STARTING_INPUTS"
    assert any(issue["code"] == "P13A032" for issue in result["issues"])
...
def test_p13a_rejects_unsupported_phase_count_and_invalid_pf():
    ...
    assert "P13A026" in codes
    assert "P13A030" in codes


def test_p13a_propagates_invalid_base_model_without_mutation():
    manifest = _manifest()
    ...
    assert result["intake_status"] == "BLOCKED_MOTOR_STARTING_INPUTS"
    assert any(issue["code"] == "P13A011" for issue in result["issues"])
    assert result["base_model_admission"]["ready_to_build_model"] is False
    assert result["electrical_calculation_performed"] is False
    assert result["model_mutation_performed"] is False


def test_p13a_study_requires_explicit_project_voltage_criterion():
    manifest = _manifest()
...
def test_p13a_contract_is_cross_industry_and_static_only():
    contract = motor_starting_intake.obtener_contrato_p13a()
...
def test_p13a_requires_explicit_source_strength_for_starting_study():
    manifest = _manifest()
...
def test_p13a_rejects_ambiguous_motor_side_or_harmonic_basis():
    manifest = _manifest()
    ...
    assert result["intake_status"] == "BLOCKED_MOTOR_STARTING_INPUTS"
    codes = {issue["code"] for issue in result["issues"]}
    assert "P13A036" in codes
    assert "P13A037" in codes
