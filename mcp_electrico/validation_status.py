...
_MODULES = {
    ...
    "tcc_curve_evaluation": {
        ...
        "limitations": [
            ...
            "No existe claim de selectividad por la mera evaluación de una curva",
            "professional_emission=false",
        ],
    },
    "protection_checks": {
    ...
    "protection_coordination": {
        "status": "EXPERIMENTAL",
        "basis": "P5E: coordinación temporal puntual conservadora usando clearing times P5D trazables",
        "limitations": [
            "Solo compara un par downstream/upstream en corrientes explícitas por dispositivo",
            "La relación upstream/downstream debe declararse con fuente; no se infiere topología",
            "Bandas: margen conservador = upstream_time_min_s - downstream_time_max_s; no se promedian",
            "No realiza barrido del dominio completo de corriente",
            "PASS/FAIL significa coordinación temporal en el punto evaluado, no selectividad total/parcial ni selectividad energética",
            "Backup y cascading permanecen NOT_EVALUATED sin tablas/evidencia específica de fabricante",
            "professional_emission=false",
        ],
    },
    "reproducible_project": {
        ...
        "limitations": [
            "P7A congela y verifica contenido mediante SHA-256 determinista",
    ...
    "technical_report": {
        "status": "EXPERIMENTAL",
        "basis": "P7C: snapshot P7A con HASH_MATCH -> contenido canónico -> SHA-256 de reporte -> HTML print-ready determinista",
        "limitations": [
            "El reporte consume únicamente datos ya congelados por P7A y no recalcula ingeniería",
            "Los estudios se separan por vigencia de revisión; resultados históricos no se promocionan",
