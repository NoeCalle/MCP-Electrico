...
FAULT_ALIASES = {
    ...
    "monofasica_tierra": "single_phase_ground",
    "monofásica_tierra": "single_phase_ground",
}
    ...
    for transformer in model.get("transformers", []):
        ...
        if not p2:
            missing.append(_item(
                "P2READY201",
                "Transformador sin ficha P2 de %Z/X-R/grupo vectorial/procedencia.",
                element,
            ))
            continue
...
def _zero_sequence_requirements() -> list[dict[str, Any]]:
    ...
    for transformer in model.get("transformers", []):
        ...
        if has_wye and neutral.get("side") is None:
            missing.append(_item(
                "P2READY421",
                "Transformador con devanado wye sin declaración explícita de neutro/puesta a tierra para la ficha Z0.",
                element,
            ))
...
def _engine_readiness(
    ...
    if preferred == "pandapower":
        ...
        if not compatibility.get("compatible"):
            return {
                "status": ENGINE_NOT_READY,
                "engine": "pandapower",
                "reasons": deepcopy(compatibility.get("issues", [])),
            }
        if str(compatibility.get("maturity")) == "EXPERIMENTAL" and not allow_experimental:
            return {
                "status": ENGINE_NOT_READY,
                "engine": "pandapower",
                "reasons": [_item("P2READY802", "pandapower permanece EXPERIMENTAL para el alcance actual; requiere habilitación explícita.")],
            }
    ...
    return {
        "status": ENGINE_NOT_READY,
        "engine": preferred or None,
        "reasons": [_item("P2READY899", "No existe una regla de preparación para el backend preferente declarado.")],
    }
