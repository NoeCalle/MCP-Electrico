"""P13B — estudio estático de arranque de motor en contexto OpenDSS aislado.

P13B consume el contrato P13A y calcula únicamente el impacto estático inicial
...
def evaluar_readiness(manifest: dict[str, Any]) -> dict[str, Any]:
    """Materializa el modelo base sin Solve y verifica el gate P13B."""
            ...
            **base,
            ...
            "issues": [
                _issue(
                    ...
                    "P13B requiere cerrar defaults retenidos relevantes antes del estudio: "
    ...
    return {
        **base,
        ...
        "note": (
            "P13B readiness materializa el modelo base pero no ejecuta Solve. "
    ...
    *,
    ...
    return {
        ...
        "equivalent_load_model": {
            ...
            "semantics": "CONSTANT_IMPEDANCE_EQUIVALENT_FROM_EXPLICIT_SUPPLY_LINE_RMS_CURRENT_AND_FUNDAMENTAL_DISPLACEMENT_PF",
...
def ejecutar_estudios(manifest: dict[str, Any]) -> dict[str, Any]:
    """Ejecuta todos los estudios P13A en contextos DSS independientes."""
    ...
    with TemporaryDirectory(prefix="mcp_electrico_p13b_") as temp:
    ...
    if not parent_preserved:
        raise RuntimeError(
            "P13BEXEC001: la ejecución aislada de arranque modificó el contexto DSS/Workspace padre."
        )
    ...
    return {
        ...
        "professional_emission": False,
        "note": (
            "P13B es una aproximación estática fundamental de impacto de red. "
            "No representa armónicos, formas de onda ni la trayectoria electromecánica completa del motor."
        ),
    }
