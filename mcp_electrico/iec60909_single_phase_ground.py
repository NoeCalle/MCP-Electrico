...
NEGATIVE_SEQUENCE_POLICY = {
    ...
    "note": "Z2=Z1 se limita al alcance pasivo simétrico actual; no se extiende a generadores, motores ni modelos asimétricos.",
}
...
def _source_zero_projection(case: str, lv_tol_percent: int = 10) -> dict[str, Any]:
    ...
    if r0 < 0 or x0 <= 0:
        raise ValueError("P4C07S003: pandapower requiere R0>=0 y X0>0 explícitos; no se aproxima X0=0.")
    ...
    return {
        **positive,
        ...
        "r0x0": r0 / x0,
        "x0x": x0 / x1_ohm,
        "mapping": {"r0x0": "R0 / X0", "x0x": "X0 / X1_backend", "preserves_absolute_z0": True},
    }


def _apply_source_zero_sequence(net, projection: dict[str, Any]) -> None:
    if len(net.ext_grid) != 1:
        raise ValueError("P4C07S010: P4C07 v1 requiere una única ext_grid.")
...
def _apply_line_zero_sequence(net) -> list[dict[str, Any]]:
    ...
    missing = [str(row["name"]) for _, row in net.line.iterrows() if str(row["name"]).lower() not in known]
    if missing:
...
def _effective_vector_group(p2: dict[str, Any], z0: dict[str, Any]) -> str:
    ...
    grounded = neutral.get("mode") in {"solid", "impedance"}
...
def _apply_transformer_zero_sequence(net) -> list[dict[str, Any]]:
    ...
    for name, idx in by_name.items():
        ...
        if missing:
            raise ValueError(f"P4C07T003: Transformer.{name} Z0 incompleto para pandapower: {', '.join(missing)}.")
        for key in required:
        ...
        if neutral.get("mode") in {"solid", "impedance"}:
    ...
    *,
    ...
    base = {
        ...
        "limitations": [
            ...
            "ip/Ith no se promocionan para 1F-T: pandapower 3.5.4 no los calcula en _calc_sc_1ph.",
    ...
    if not compatibility.get("compatible"):
        return {**base, "ok": False, "status": "NOT_COMPATIBLE", "issues": deepcopy(compatibility.get("issues", []))}

    model = pandapower_engine._collect_active_model()
    if not model.get("source"):
        return {**base, "ok": False, "status": "MISSING_SOURCE_DATA", "issues": [_issue("P4C07S001", "Falta red equivalente P2.")]}

    source_projection, source_issues = iec60909._source_projection(case_norm)
    if source_issues or not source_projection:
        return {**base, "ok": False, "status": "MISSING_SOURCE_DATA", "issues": deepcopy(source_issues)}

    temperatures, temperature_issues = iec60909._line_temperature_map(model, case_norm, line_endtemp_degree_c)
    ...
    if target.lower().startswith("bus."):
        target = target[4:]
    matches = [int(idx) for idx, row in net.bus.iterrows() if str(row["name"]).lower() == target.lower()]
    if not matches:
        return {
            **base,
            "ok": False,
            ...
            "inputs": {"source_projection": source_projection, "zero_sequence_projection": zero_projection, "line_temperature_projection": temperatures},
        }
    pp_bus = matches[0]

    try:
        pp.shortcircuit.calc_sc(net, bus=pp_bus, fault="1ph", case=case_norm, lv_tol_percent=int(lv_tol_percent), ip=False, ith=False)
    ...
    return {
        **base,
        ...
        "backend_raw": {"skss_mw": value("skss_mw"), "columns": [str(col) for col in net.res_bus_sc.columns]},
