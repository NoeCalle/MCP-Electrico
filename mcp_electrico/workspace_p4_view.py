...
Consume exclusivamente el estudio versionado ``iec60909_3ph``. El navegador
no ejecuta pandapower, no deriva corrientes y no completa escenarios fallidos.
"""
...
def _study(snapshot: dict[str, Any]) -> dict[str, Any] | None:
    item = snapshot.get("status", {}).get("studies", {}).get("iec60909_3ph")
...
Ejecuta explícitamente el estudio 3F MAX/MIN para registrar resultados P4 en esta revisión.</div>
</section>'''
    ...
    fault = str(study.get("fault") or "3ph").upper()
    ...
    return f'''<section class="panel p4-panel" id="panel-cortocircuito" data-p4-fault-bus="{escape(bus, quote=True)}">
<div class="p4-header">
  <div><h3>Cortocircuito — IEC 60909</h3><p>Falla {escape(fault)} en barra <strong>{escape(bus)}</strong> · resultados registrados para la revisión vigente.</p></div>
  <div class="p4-kpis">
...
.p4-header h3 { margin:0 0 4px; color:var(--blue); font-size:16px; }
...
@media print { .p4-panel { display:block !important; break-inside:avoid; } }
'''
...
(() => {
  ...
  if (!panel || !tab) return;
  const faultBus = panel.dataset.p4FaultBus;
  ...
  function applyOverlay() {
    clearOverlay();
    if (!faultBus || faultBus === '—') return;
    nodesFor(`Bus.${faultBus}`).forEach(node => node.classList.add('overlay-short-circuit-bus'));
  }
...
def enhance_html(html: str, snapshot: dict[str, Any]) -> str:
    """Añade la primera vista V4 IEC 60909 de forma idempotente."""
    if MARKER in html:
        return html
