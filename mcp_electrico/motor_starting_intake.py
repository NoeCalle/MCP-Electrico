"""P12A — contrato fail-closed para motores y estudios de arranque.
...
SCHEMA = "MCP_ELECTRICO_P12A_MOTOR_STARTING_INTAKE_V1"
...
def evaluar_admision_motor(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("id", "name", "source_reference"):
        if not _present(project.get(key)):
            issues.append(_issue("P12A001", f"project.{key}", f"project.{key} es requerido."))

    base_model = payload.get("base_model")
    if not isinstance(base_model, dict):
        issues.append(_issue("P12A010", "base_model", "Se requiere un manifiesto base P8 estructurado."))
        base_admission = None
        buses: set[str] = set()
        loads: dict[str, dict[str, Any]] = {}
    else:
    ...
    if not isinstance(motors, list) or not motors:
        issues.append(_issue("P12A020", "motors", "Se requiere al menos un motor explícito."))
        motors = []
    ...
    for index, raw in enumerate(motors):
        ...
        if not isinstance(raw, dict):
            issues.append(_issue("P12A021", path, "Cada motor debe ser un objeto estructurado."))
            continue

        identifier = _motor_id(raw.get("id"))
        if not identifier:
            issues.append(_issue("P12A022", f"{path}.id", "El motor requiere id."))
        elif identifier.lower() in seen_motors:
            issues.append(_issue("P12A023", f"{path}.id", "No se permiten motores duplicados."))
        else:
        ...
        for key in (
            ...
            if not _present(raw.get(key)):
                issues.append(_issue("P12A024", f"{path}.{key}", f"Dato de motor requerido: {key}."))

        bus = str(raw.get("bus") or "").strip()
        if bus and buses and bus not in buses:
            issues.append(_issue("P12A025", f"{path}.bus", "La barra del motor no existe en base_model.topology.buses."))
        ...
        if phases is not None and phases != 3:
            issues.append(_issue(
                "P12A026",
                f"{path}.phases",
                "P12 v1 limita el estudio estático de arranque a motores trifásicos.",
            ))

        for key in ("kv_ll", "rated_output_kw", "starting_current_a"):
            if _present(raw.get(key)) and not _positive(raw.get(key)):
                issues.append(_issue("P12A027", f"{path}.{key}", f"{key} debe ser finito y mayor que cero."))

        connection = str(raw.get("connection") or "").strip().lower()
        if connection and connection not in ALLOWED_CONNECTIONS:
            issues.append(_issue("P12A028", f"{path}.connection", "connection debe ser wye o delta."))
        ...
        if method and method not in ALLOWED_STARTING_METHODS:
            issues.append(_issue(
                ...
                "Método de arranque no soportado por el contrato P12A.",
            ))
    ...
    if not isinstance(studies, list) or not studies:
        issues.append(_issue("P12A040", "studies", "Se requiere al menos un estudio de arranque."))
        studies = []
    ...
    for index, raw in enumerate(studies):
        ...
        if not isinstance(raw, dict):
            issues.append(_issue("P12A041", path, "Cada estudio debe ser un objeto estructurado."))
            continue
        for key in ("id", "motor_id", "study_type", "minimum_terminal_voltage_pu", "criterion_reference"):
            if not _present(raw.get(key)):
                issues.append(_issue("P12A042", f"{path}.{key}", f"Dato de estudio requerido: {key}."))

        sid = str(raw.get("id") or "").strip()
        if sid:
            if sid.lower() in study_ids:
                issues.append(_issue("P12A043", f"{path}.id", "ID de estudio duplicado."))
            study_ids.add(sid.lower())

        motor_id = _motor_id(raw.get("motor_id"))
        if motor_id and motor_id.lower() not in referenced_motors:
            issues.append(_issue("P12A044", f"{path}.motor_id", "El estudio referencia un motor inexistente."))
