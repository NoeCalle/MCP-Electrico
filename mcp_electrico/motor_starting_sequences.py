...
Cada paso declara el estado completo de todos los motores P13 del manifiesto.
Los pasos se ordenan por elapsed_time_s, pero el tiempo no entra al solver.
Cada paso se reconstruye en un OpenDSS NewContext fresco: no existe transición
implícita, memoria dinámica, interpolación ni selección automática de arranque.
"""
...
from . import motor_starting_profiles, motor_starting_static
...
def obtener_contrato_p13d() -> dict[str, Any]:
    return {
        ...
        "all_manifest_motors_must_be_declared": True,
...
def validar_secuencias(
    ...
    """Valida secuencias multi-motor sin construir ni resolver la red."""
    ...
    for i, raw in enumerate(sequences):
        ...
        declared_set = set(motor_keys)
        if declared_set != all_motor_keys:
            missing = sorted(all_motor_keys - declared_set)
            extra = sorted(declared_set - all_motor_keys)
            issues.append(_issue(
                "P13D012",
                f"{path}.motor_ids",
                "P13D v1 exige declarar todos los motores P13 del manifiesto; "
        ...
        for j, motor_id in enumerate(motor_ids):
            ...
            if not str(motor.get("running_load_element_id") or "").strip():
                issues.append(_issue(
                    "P13D015",
                    f"{path}.motor_ids[{j}]",
                    "El motor requiere running_load_element_id explícito.",
                ))
        ...
        for j, step in enumerate(steps):
            ...
            if not isinstance(step, dict):
                issues.append(_issue("P13D018", spath, "Cada paso debe ser un objeto."))
                continue
            ...
            if not step_id:
                issues.append(_issue("P13D019", f"{spath}.id", "Cada paso requiere id."))
            elif step_id.lower() in seen_step_ids:
                issues.append(_issue("P13D020", f"{spath}.id", "ID de paso duplicado."))
            else:
            ...
            for k, state in enumerate(states):
                ...
                if not isinstance(state, dict):
                    issues.append(_issue("P13D025", mpath, "Cada estado de motor debe ser un objeto."))
                    continue
                ...
                if not motor_id or motor_key not in declared_set:
                    issues.append(_issue(
                        "P13D026",
                        f"{mpath}.motor_id",
            ...
            if state_keys != declared_set or len(normalized_states) != len(motor_ids):
                missing_states = sorted(declared_set - state_keys)
                issues.append(_issue(
                    "P13D034",
                    f"{spath}.motor_states",
                    "Cada paso debe declarar exactamente un estado para cada motor; "
                    f"faltan={missing_states}.",
                ))
...
def _study_by_motor(studies: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    result: dict[str, dict[str, Any]] = {}
    for study in studies.values():
        motor_id = str(study.get("motor_id") or "").lower()
        if motor_id:
            result[motor_id] = study
    return result
...
def _execute_step(
    *,
    ...
    motors_list: list[dict[str, Any]],
    bus_bases: dict[str, float],
    step: dict[str, Any],
    motors: dict[str, dict[str, Any]],
    studies_by_motor: dict[str, dict[str, Any]],
) -> dict[str, Any]:
    engine, _ = motor_starting_static._build_isolated_base(
        base_model,
        motors_list,
        bus_bases,
    )
    ...
    starting_motor_keys: list[str] = []

    for state in step["motor_states"]:
        motor_key = state["motor_id"].lower()
        motor = motors[motor_key]
        running = str(motor["running_load_element_id"])
        ...
        if state["state"] == STATE_STARTING:
            ...
            name = (
                "p13d_"
                + motor_starting_static._safe_element_suffix(step["id"])
                + "_"
                + motor_starting_static._safe_element_suffix(motor["id"])
            )
            engine(
                " ".join([
                    f"New Load.{name}",
            ...
            applied["starting_point"] = deepcopy(point)
            applied["starting_demand"] = demand
            starting_motor_keys.append(motor_key)
    ...
    participant_voltages: list[dict[str, Any]] = []
    voltage_by_motor: dict[str, list[float]] = {}
    for motor_key, motor in motors.items():
        values = motor_starting_static._bus_voltage_pu(engine, motor["bus"])
        voltage_by_motor[motor_key] = values
        participant_voltages.append({
            "motor_id": motor["id"],
            "bus": motor["bus"],
            "voltage_pu_by_phase": values,
            "minimum_voltage_pu": min(values) if values else None,
        })

    if any(not values for values in voltage_by_motor.values()):
        return {
            "step_id": step["id"],
            "elapsed_time_s": step["elapsed_time_s"],
            "status": "PARTICIPANT_VOLTAGE_NOT_READY",
            "ok": False,
            "applied_states": applied_states,
            "participant_voltages": participant_voltages,
            "elapsed_time_used_by_solver": False,
            "professional_emission": False,
        }

    criterion_evaluations: list[dict[str, Any]] = []
    for motor_key in starting_motor_keys:
        motor = motors[motor_key]
        study = studies_by_motor[motor_key]
        values = voltage_by_motor[motor_key]
        minimum = min(values)
        ...
        criterion_evaluations.append({
            ...
            "criterion_reference": study["criterion_reference"],
            "passed": minimum >= criterion,
            "universal_normative_claim": False,
        })

    minimum_participant = min(
        item["minimum_voltage_pu"]
        for item in participant_voltages
        if item["minimum_voltage_pu"] is not None
    )
    if criterion_evaluations:
        passed = all(item["passed"] for item in criterion_evaluations)
    ...
    return {
        ...
        "applied_states": applied_states,
        "participant_voltages": participant_voltages,
        "minimum_participant_voltage_pu": minimum_participant,
...
def ejecutar_secuencias(
    ...
    """Ejecuta cada paso desde una reconstrucción limpia e independiente."""
    ...
    intake = p13c["p13a"]
    motors_list = deepcopy(intake.get("motors") or [])
    motors = _motor_map(p13c)
    studies = _study_map(p13c)
    studies_by_motor = _study_by_motor(studies)
    base_model = deepcopy(manifest["base_model"])
    preflight, bus_bases = motor_starting_static._preflight_base(base_model, motors_list)
    if preflight:
        raise RuntimeError("P13DEXEC000: readiness y ejecución discrepan en preflight.")
    ...
    for sequence in validation["sequences"]:
        step_results = [
            _execute_step(
                ...
                step=step,
                motors=motors,
                studies_by_motor=studies_by_motor,
            )
            for step in sequence["steps"]
        ]

        solved = bool(step_results) and all(item.get("ok") is True for item in step_results)
        any_fail = any(item.get("status") == "FAIL" for item in step_results)
        all_evaluations = [
            {
                **deepcopy(evaluation),
        ...
        worst = (
            min(all_evaluations, key=lambda item: item["minimum_voltage_pu"])
            if all_evaluations
            else None
        )
        ...
        sequence_results.append({
            ...
            "status": "PARTIAL" if not solved else ("FAIL" if any_fail else "PASS"),
            "ok": solved,
    ...
    if parent_after != parent_before:
        raise RuntimeError(
            "P13DEXEC001: secuencias aisladas modificaron el contexto DSS/Workspace padre."
        )
